...
def main():
    project = angr.Project("./re30")

    #create an initial state with a symbolic bit vector as argv1
    argv1 = claripy.BVS("argv1",12*8) #since we do not the length now, we just put 100 bytes
    initial_state = project.factory.entry_state(args=["./re30",argv1])
    ...
    sm.explore(find=0x40064a) #at this instruction the binary will print the "correct" message
